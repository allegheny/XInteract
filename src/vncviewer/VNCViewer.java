...
public class VNCViewer extends java.applet.Applet implements Runnable
{
  ...
  public void start() {
    ...
    //System.out.println("debug: starting thread");
    thread.start();
  }
  ...
  // AC: make cc a field so that it can be externally referenced
  public CConn cc;
  public boolean ioHasBeenSet = false;
  ...
  public void run() {
    ...
    try {
      ...
      if (cc.init(null, vncServerName.getValue(),
    	  ...
    	  ioHasBeenSet = true;
        while (true) {
        	//if (isBig) System.out.println("debug: processing message");
          cc.processMsg();
        }
      }
